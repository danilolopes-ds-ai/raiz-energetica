...
const BlogPostCard = ({ post, index, getCategoryName, formatDate, isFeaturedLayout = false }) => {
  return (
    <motion.div
      key={post.id}
    ...
    >
      <Card className="card-hover h-full overflow-hidden">
        ...
        <CardContent>
          <p className={`text-gray-600 mb-4 ${isFeaturedLayout ? '' : 'text-sm line-clamp-3'}`}>{post.excerpt}</p>
          <Button 
            asChild
            variant="outline"
            size={isFeaturedLayout ? 'default' : 'sm'}
            className={`border-[#2D8C5C] text-[#2D8C5C] hover:bg-[#2D8C5C] hover:text-white ${isFeaturedLayout ? '' : 'w-full'}`}
          >
            <Link to={`/blog/${post.id}`}>
              Ler Artigo
